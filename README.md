...
## 请在master下载raw完整的项目压缩包！

## Vivado 2024.2

## 项目概述

该项目基于 ZYNQ-7100 平台，实现了两套 YOLOv3-Tiny 的硬件加速方案，集成了摄像头数据采集、(去雾算法)FPGA 推理和 HDMI 显示回显的完整流程。该方案通过软硬件协同，充分利用 FPGA 的并行计算优势，提高了目标检测的速度和效率，同时降低了功耗。
...
基本复杂场景下的推理：通过去雾版本的推理可以基本完成雾气条件下的模型算法。
...
Q7:如何去雾？
A:本工程通过按键的控制操作来进行是否去雾，可根据自己板子的key在xdc中配置。
